...
/* ---- ADHDBP database auth (lockdown): Worker signs in privately; app/SW use APP_TOKEN ---- */
let dTok="", dRef="", dExp=0;
async function dbAuth(env){
  if(!env.DB_API_KEY || !env.DB_EMAIL || !env.DB_PASSWORD) return false;
  if(dTok && Date.now()<dExp-60000) return true;
  try{
    if(dRef){
      const r=await fetch("https://securetoken.googleapis.com/v1/token?key="+env.DB_API_KEY,{method:"POST",headers:{"Content-Type":"application/x-www-form-urlencoded"},body:"grant_type=refresh_token&refresh_token="+encodeURIComponent(dRef)});
      if(r.ok){ const j=await r.json(); dTok=j.id_token; dRef=j.refresh_token; dExp=Date.now()+(parseInt(j.expires_in,10)||3600)*1000; return true; }
    }
    const r=await fetch("https://identitytoolkit.googleapis.com/v1/accounts:signInWithPassword?key="+env.DB_API_KEY,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({email:env.DB_EMAIL,password:env.DB_PASSWORD,returnSecureToken:true})});
    if(r.ok){ const j=await r.json(); dTok=j.idToken; dRef=j.refreshToken; dExp=Date.now()+(parseInt(j.expiresIn,10)||3600)*1000; return true; }
  }catch(e){}
  dTok=""; return false;
}
async function dbUrl(env, node){
  const base=(env.FB_URL||"").replace(/\/$/,"")+"/"+node+".json";
  const ok=await dbAuth(env);
  return ok ? base+"?auth="+encodeURIComponent(dTok) : base;
}
function tokOK(req, url, env){
  if(!env.APP_TOKEN) return false;
  const t=req.headers.get("X-TD-Key") || url.searchParams.get("t") || "";
...
async function runReminders(env, force){
  ...
  const res=await fetch(await dbUrl(env,FB_PATH),{cache:"no-store"});
  if(!res.ok) return;
    ...
    try{
      ...
      if(h===cfg.summary.hour){
        ...
        if(!seen){
          ...
          await fetch(await dbUrl(env,"_summary"),{method:"PUT",body:JSON.stringify({ms:now})});
          summaryPing=true;
        }
      }
    }catch(e){}
...
export default {
  async fetch(req, env){
   try{
    ...
    if(url.pathname==="/tasks" || url.pathname==="/ideas"){
      if(!tokOK(req,url,env)) return new Response(JSON.stringify({error:"bad_token"}),{status:401,headers:{...cors,"Content-Type":"application/json"}});
      const node = url.pathname==="/tasks" ? FB_PATH : "td_ideas";
      const u=await dbUrl(env,node);
      if(req.method==="GET"){
        const r=await fetch(u,{cache:"no-store"});
      ...
      if(req.method==="PUT" || req.method==="POST"){
        ...
        return new Response(JSON.stringify({ok:r.ok}),{status:r.ok?200:502,headers:{...cors,"Content-Type":"application/json"}});
      }
    }
    if(url.pathname==="/summary" && req.method==="GET"){
      if(!tokOK(req,url,env)) return new Response(JSON.stringify({error:"bad_token"}),{status:401,headers:{...cors,"Content-Type":"application/json"}});
      const r=await fetch(await dbUrl(env,"_summary"),{cache:"no-store"});
      return new Response(await r.text(),{status:r.status,headers:{...cors,"Content-Type":"application/json"}});
    }
